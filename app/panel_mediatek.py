...
from core.engine import MotorMtk

COLOR_FONDO = "#21201e"
COLOR_SUB = "#16191c"
COLOR_SUB2 = "#12151a"

class PanelMediaTek(ctk.CTkFrame):
    def __init__(self, parent):
        super().__init__(parent, fg_color=COLOR_FONDO)
        self.db = config.cargar_modelos_mtk()
        self.marca_actual = None
        self.modelo_actual = None
        self.motor = MotorMtk(self._log, self._status)
        ...
        self.label_detalle = ctk.CTkLabel(self.frame_detalle, text="Selecciona un modelo de la lista", 
                                           font=("Consolas", 14), text_color="#c9ced3", justify="left", anchor="nw")
        ...
        # Panel de ejecución (abajo, global)
        self.frame_accion = ctk.CTkFrame(self, fg_color=COLOR_SUB, corner_radius=8)
        self.frame_accion.pack(fill="x", padx=10, pady=(0, 8))
        self.label_comando = ctk.CTkLabel(self.frame_accion, text="mtk ...", font=("Consolas", 12), text_color="#c9ced3")
    ...
    def seleccionar_modelo(self, m):
        ...
        datos = [
            ...
            ("COMANDO", f"{m.get('comando_base','--')}"),
            ("LOADER", f"{m.get('loader','off')}"),
            ("INSTRUCCIONES", f"{m.get('instruccion','')}"),
        ]
        texto = "\n".join(f"> {k}: {v}" for k, v in datos)
        self.label_detalle.configure(text=texto)

        loader = config.preparar_loader(m.get('loader'))
        cmd = self.motor.construir_comando(m.get('comando_base', 'e frp'), loader)
    ...
    def _ejecutar(self):
        ...
        cmd = self.motor.construir_comando(self.modelo_actual.get('comando_base', 'e frp'), loader)
        if not self.motor.accion_disponible():
            self._log("[ERROR] mtkclient no está instalado (mirar panel Drivers)")
            return
        self.boton_ejecutar.configure(state="disabled")
        self.texto_consola.delete("1.0", "end")
        def run():
            rc = self.motor.ejecutar(cmd, timeout_espera=None)
